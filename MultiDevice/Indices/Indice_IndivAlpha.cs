...
namespace MultiDevice
{
    ...
    class Indice_IndivAlpha : IndiceToBytes
    {
        const int sampling_rate = 2000;
        const int num200ms_for_cal = 10;  // 8s:200ms*40

        const int re_fs_ratio = 8;// downsample 1000 Hz to 200Hz
        ...
        public Indice_IndivAlpha(List<int> channels_involved)
        {
            ...
            tmp_var.Width =(int) Math.Pow(2,Math.Ceiling(Math.Log(sampling_rate)/Math.Log(2)));
            hamming_window = tmp_var.CopyToArray();
            var tmp_var2 = (new MathNet.Filtering.Windowing.HammingWindow());
            tmp_var2.Width = (int)Math.Pow(2, Math.Ceiling(Math.Log(sampling_rate) / Math.Log(2)))/ re_fs_ratio;
        ...
        public double[,] Data200ms
        {
            ...
            set
            {
                ...
                if (true)
                {
                    ...
                    current_index200ms++;
                    Console.WriteLine("current sec (200ms):" + (current_index200ms / 5));
                ...
                else
                {
                    ...
                    CalculationDone?.Invoke(this, new MyEventArgs_indice(indices_enu.indiv_alpha));
                }
            }
        }
        ...
        void cal_ans()
        {
            ...
            else
            {
                ...
                CalculationDone?.Invoke(this, new MyEventArgs_indice(indices_enu.indiv_alpha));
                // LogHelper.Log.LogInfo($"Indice_IndivAlpha 数据推送:{indices_enu.indiv_alpha}");
            }
        ...
        float[] power_ratio(double[] real, double[] img, int fs)
        {
            ...
            double[] freqs = new double[freqs_len];
            double[] freqs_real = new double[freqs_len];
            double[] psds = new double[freqs_len];
            double[] psds_real = new double[freqs_len];
            double[] psds_diff = new double[freqs_len];
            int kkk = 0;
            for (int i = units_length * 1; i < 45 * units_length; i++)
            {
                freqs[kkk] = Math.Log10(i / 1.0 / units_length / 1.0);
                freqs_real[kkk] = i / 1.0 / units_length / 1.0;
                psds[kkk] = Math.Log10(abs_value[i]);
                psds_real[kkk] = abs_value[i];
            ...
            {
                if (freqs_real[i] > 7 && freqs_real[i] < 14)
                {
                    //sum_1 += psds_diff[i] * freqs_real[i];
                    //sum_2 += psds_diff[i];
                    //sum_1 += psds[i] * freqs_real[i];
                    //sum_2 += psds[i];
                    sum_1 += psds_real[i] * freqs_real[i];
                    sum_2 += psds_real[i];
                }
            }
            double indiv_alpha_freq = sum_1 / sum_2;



            //var p = Fit.Line(freqs_real, psds);
            //for (int i = 0; i < freqs_len; i++)
            //{
            //    psds_diff[i] = psds[i] - (p.Item1 + p.Item2 * freqs_real[i]);
            //}
            //int max_idx = 0;//max idx in 6-12Hz  as the peak alpha frequency
            ...
            //    }
            //}
            //double indiv_alpha_freq = freqs_real[max_idx];


            Console.WriteLine("indiv_alpha_freq:" + indiv_alpha_freq);

            double alpha_p = 0, alpha_a = 0, low_alpha_p = 0, low_alpha_a = 0, high_alpha_p = 0, high_alpha_a = 0;
            for (int i = (int)(units_length * indiv_alpha_freq * 0.8); i < (int)(units_length * indiv_alpha_freq * 1.2); i++)
            {
                alpha_p += abs_value[i];
            }
            alpha_a = (alpha_p / (indiv_alpha_freq * 0.4 * units_length)) / (all_power / (44 * units_length));
            Console.WriteLine("pass 1");
            for (int i = (int)(units_length * indiv_alpha_freq * 0.8); i < (int)(units_length * indiv_alpha_freq * 1); i++)
            {
                low_alpha_p += abs_value[i];
            }
            low_alpha_a = (low_alpha_p / (indiv_alpha_freq * 0.2 * units_length)) / (all_power / (44 * units_length));
            Console.WriteLine("pass 2");
            for (int i = (int)(units_length * indiv_alpha_freq * 1); i < (int)(units_length * indiv_alpha_freq * 1.2); i++)
            {
                high_alpha_p += abs_value[i];
            }
            high_alpha_a = (high_alpha_p / (indiv_alpha_freq * 0.2 * units_length)) / (all_power / (44 * units_length));
            ...
            ratios[7] = (float)indiv_alpha_freq;
