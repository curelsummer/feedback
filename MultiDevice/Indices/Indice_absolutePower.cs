...
namespace MultiDevice
{
    ...
    class Indice_absolutePower : IndiceToBytes
    {
     const   int sampling_rate = 2000;
        ...
        public Indice_absolutePower(List<int> channels_involved)
        {
        ...
        { 
            set
            {
                ...
                else
                {
                    ...
                    CalculationDone?.Invoke(this, new MyEventArgs_indice(indices_enu.absolute_power));
                }
            }
        }
        ...
        void cal_ans()
        {
            ...
            else
            {
                ...
                CalculationDone?.Invoke(this, new MyEventArgs_indice(indices_enu.absolute_power));
                // LogHelper.Log.LogInfo($"Indice_absolutePower 数据推送:{indices_enu.absolute_power}");
            }
        }
        ...
        float[] power_ratio(double[] real, double[] img, int fs)
        {
            ...
            //alpha 7-13 Hz
            for (int i = (int)IAF_low * units_length; i < (int)IAF_high * units_length; i++)
            {
                alpha += abs_value[i];
            }
            ...
            ratios[0] = (float)delta;
            ratios[1] = (float)theta;
            ratios[2] = (float)low_alpha;
            ratios[3] = (float)alpha;

            ratios[4] = (float)SMR;
            ratios[5] = (float)low_beta;
            ratios[6] = (float)beta;
            ratios[7] = (float)gamma;
            return ratios;
        }
        ...
        public byte[] toBytes()
        {
            ...
            Console.WriteLine("float bytes length:" + BitConverter.GetBytes(4.5f).Length);

            //前两位是方法的enumeration代码，见：utilities.cs,注意字节序，是高位在后
            c[0] = Convert.ToByte(0);
